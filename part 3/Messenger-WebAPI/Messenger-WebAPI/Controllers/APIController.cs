...
namespace Messenger_WebAPI.Controllers
{
    ...
    public class APIController : Controller
    {
        ...
        public static Dictionary<string, string> usersToken = new Dictionary<string, string>();
        ...
        public async Task<IActionResult> InvitationsPOST(Invitation invitation)
        {
            ...
            // check if user has token, and if so, send notification to firebase.
            string token = "";
            bool hasToken = usersToken.TryGetValue(user.Id, out token);
            if (hasToken)
            {
                string response = FirebaseMessaging.DefaultInstance.SendAsync(new FirebaseAdmin.Messaging.Message()
                {
                    Data = new Dictionary<string, string>()
                {
                    ...
                    {
                        Title = contact.Name,
                        Body = contact.Name + " wants to chat with you.",
                    },
